...
/* Onemogućiti scroll dok je navigacija na manjim ekranima otvorena */
...
function init() {
  ...
  ScrollTrigger.batch(".anim-down", {
    start: "top center",
    onEnter: (batch) =>
      gsap.to(batch, { opacity: 1, y: 0, duration: 1, ease: "Power4.out", overwrite: true }),
  ...
  ScrollTrigger.batch(".anim-down-link", {
    start: "top bottom",
    onEnter: (batch) =>
      gsap.to(batch, { opacity: 1, y: 0, duration: 1, stagger: 0.3, ease: "Power4.out", overwrite: true }),
    onLeave: (batch) => gsap.set(batch, { opacity: 1, y: 0, overwrite: true }),
  });
